...
#define JNI_METHOD(return_type, method_name) \
  JNIEXPORT return_type JNICALL              \
      Java_com_taotianhua_cardboard_VrActivity_##method_name


inline jlong jptr(MazeGame *native_app) {
    return reinterpret_cast<intptr_t>(native_app);
}

inline MazeGame *native(jlong ptr) {
    return reinterpret_cast<MazeGame *>(ptr);
}
...
(JNIEnv *env, jobject obj, jobject asset_mgr) {
    return jptr(new MazeGame(javaVm, obj, asset_mgr));
}
...
JNI_METHOD(void, nativeUpdateTouching)
(JNIEnv *env, jobject obj, jlong native_app, jboolean touching) {
    native(native_app)->UpdateTouching(touching);
}
